...
function jobFromSheet(row) {
  ...
  const base = {
    ...
    name: String(row[3] || ''), phone, address: String(row[5] || ''),
...
async function loadJobsFromSheet(showMessage = true) {
  ...
  const response = await sheetsRequest(`${base}/${encodeURIComponent(`${SHEET_NAME}!A2:Q`)}?valueRenderOption=FORMULA&dateTimeRenderOption=FORMATTED_STRING`);
...
async function syncJob(id, options = {}) {
  ...
  try {
    ...
    if (rowIndex >= 0) {
      ...
      await sheetsRequest(`${base}/${encodeURIComponent(`${SHEET_NAME}!A${rowNumber}:Q${rowNumber}`)}?valueInputOption=RAW`, {
        method: 'PUT', body: JSON.stringify({ values: sheetRow(job) })
      });
    } else {
      await sheetsRequest(`${base}/${encodeURIComponent(`${SHEET_NAME}!A:Q`)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`, {
